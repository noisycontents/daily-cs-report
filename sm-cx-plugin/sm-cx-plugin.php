...
function sm_cx_return_daily_stats() {
  ...
  if (class_exists('WooCommerce')) {
    // 모든 주문을 가져온 후 결제 시간으로 필터링 (모든 상태 포함)
    $all_orders = wc_get_orders([
      'limit' => -1, // 모든 주문 가져오기
      'date_created' => '>=' . date('Y-m-d', strtotime('-30 days')) // 한 달 내 주문 (성능 최적화)
    ]);
    
    // 결제 시간 기준으로 필터링 (정확히 해당 날짜만)
    $orders = [];
    $target_date_start = strtotime($yesterday . ' 00:00:00');
    $target_date_end = strtotime($yesterday . ' 23:59:59');
    
    foreach ($all_orders as $order) {
      $paid_date = $order->get_date_paid();
      if ($paid_date) {
        $paid_timestamp = $paid_date->getTimestamp();
        $paid_date_str = $paid_date->format('Y-m-d');
        
        // 정확히 해당 날짜에 결제된 주문만 포함
        if ($paid_date_str === $yesterday) {
          $orders[] = $order;
        }
      } else {
        // 결제 시간이 없는 경우, 주문 완료 시간 → 생성 시간 순으로 확인
        $completed_date = $order->get_date_completed();
        $created_date = $order->get_date_created();
        
        // 1순위: 주문 완료 시간이 해당 날짜인지 확인
        if ($completed_date && $completed_date->format('Y-m-d') === $yesterday) {
          $orders[] = $order;
        }
        // 2순위: 완료 시간이 없거나 다른 날짜면 주문 생성 시간 확인
        else if ($created_date && $created_date->format('Y-m-d') === $yesterday) {
          $orders[] = $order;
        }
      }
    }
    ...
    foreach ($orders as $order) {
      ...
      // 결제 완료 시간 가져오기 (없으면 주문 생성 시간 사용)
      $paid_date = $order->get_date_paid();
      $order_date = $paid_date ? $paid_date->format('Y-m-d H:i:s') : $order->get_date_created()->format('Y-m-d H:i:s');
      ...
      if ($order_total > 0) {
        ...
        // 신규/기존 회원 구분 (10,000원 이상 주문만 집계, 중복 방지)
        if ($order_total >= 10000) {
          $customer_id = $order->get_customer_id();
          
          if ($customer_id > 0) {
            // 회원인 경우: 이미 처리된 고객인지 확인
            if (!in_array($customer_id, $processed_customers)) {
              $processed_customers[] = $customer_id;
              
              // 모든 이전 구매를 확인하여 첫 구매인지 판단 (환불 제외, 결제 완료 시간 기준)
              $all_previous_orders = wc_get_orders([
                'customer_id' => $customer_id,
                'status' => ['wc-completed', 'wc-processing', 'wc-shipping', 'wc-shipped'],
                'limit' => -1,
                'meta_query' => [
                  [
                    'key' => '_order_total',
              ...
              // 결제 완료 시간 기준으로 이전 주문 필터링
              $previous_paid_orders = [];
              $target_start = strtotime($yesterday . ' 00:00:00');
              
              foreach ($all_previous_orders as $prev_order) {
                $prev_paid_date = $prev_order->get_date_paid();
                if ($prev_paid_date) {
                  $prev_paid_timestamp = $prev_paid_date->getTimestamp();
                  if ($prev_paid_timestamp < $target_start) {
                    $previous_paid_orders[] = $prev_order;
                  }
                }
              }
              ...
              if (empty($previous_paid_orders)) {
                ...
                // 이전에 구매 경험 있음 (10,000원 이상)
                $returning_orders++;
              }
            }
          } else {
            // 비회원 주문은 기존 고객으로 처리 (10,000원 이상만)
            // 비회원은 중복 체크 불가능하므로 매번 카운트
            $returning_orders++;
          }
        }
      }
    ...
    $refunds = wc_get_orders([
      ...
      'date_created' => $yesterday . 'T00:00:00...' . $yesterday . 'T23:59:59',
      'limit' => -1
    ]);
